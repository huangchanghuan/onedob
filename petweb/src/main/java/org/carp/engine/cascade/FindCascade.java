...
public class FindCascade implements Cascade{
	...
	private Object _key;
	public FindCascade(AbstractCarpSession session, CarpBean bean, Object data, Object key){
		...
		if(otms != null)
			for(OTMMetadata otm : otms){
				if(otm.getCascade()== CarpAnnotation.Cascade.All || otm.getCascade()== CarpAnnotation.Cascade.Load){
					String sql = _session.getJdbcContext().getContext().getCarpSql(otm.getChildClass()).getQuerySql();
